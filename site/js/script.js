...
$document.ready(function () {
	...
	if (plugins.calendar.length) {
		...
		for (i = 0; i < plugins.calendar.length; i++) {
			...
			// Rdc Open PopUp
			const rdcOpenEvent = (rdc) => {
			for (let i = 0; i < rdc.length; i++) {
				let rdcItem = $(rdc[i]);

				rdcItem.find($('.rdc-table_date')).on("click", function () {
					let allTableHasEvent = $('.rdc-table_has-events');
					let allTableEventCounter = $('.rdc-table_events-count');

					let tableEvents = rdcItem.find($('.rdc-table_events'));
					let ch = tableEvents.outerHeight();
					let tableEventCounter = rdcItem.find($('.rdc-table_events-count'));

					if (rdcItem.hasClass('opened')) {
						allTableHasEvent.removeClass('opened');
						allTableEventCounter.css({ height: 0 });
						return false;
					}
					...
					rdcItem.addClass("opened");
					tableEventCounter.css({ height: ch + 'px' });
				});
			}
		};
			...
			// Rdc Resize
			const rdcResizeEvent = (rdc) => {
				$(window).on('resize', function () {
					for (let i = 0; i < rdc.length; i++) {
						let rdcItem = $(rdc[i]);
						let rdcItemOpened = rdcItem.hasClass('opened');
						let tableEvents = rdcItem.find($('.rdc-table_events'));
						let ch = tableEvents.outerHeight();
						let tableEventCounter = rdcItem.find($('.rdc-table_events-count'));
			...
			rdcOpenEvent($('.rdc-table_has-events'));
			rdcResizeEvent($('.rdc-table_has-events'));
			rdcRadio();
